...
def move_crate(from_stack, to_stack, stacks_list):
    """Move one crate from_stack, to_stack"""
    popped = stacks_list[from_stack].get()
    stacks_list[to_stack].put(popped)
...
def perform_instruction(line_of_text, stacks_list):
    ...
    for i in range(pop):
        move_crate(from_stack, to_stack, stacks_list)
